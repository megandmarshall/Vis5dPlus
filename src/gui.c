...
static char *Copyright[] = {
   " Vis5D version " VERSION "  Copyright (C) 1990 - 2000 ",
...
#ifdef SLOW_VOLUME_RENDER
		{
		  /* JPE: volume is turned off during motion */
		  /* has no effect if volume is not already set */
		  int i, ctxcnt, cindex[VIS5D_MAX_DPY_CONTEXTS];
		  vis5d_get_num_of_ctxs_in_display(index, &ctxcnt, cindex);
		  for(i=0;i<ctxcnt;i++){
			 vis5d_enable_graphics(cindex[i], VIS5D_VOLUME, -2, VIS5D_OFF);
		  }
		}
#endif
   ...
   /*** Button release ***/
      ...
      if (ev->xbutton.button == Button1) {
...
#ifdef SLOW_VOLUME_RENDER
		{
		  ...
		  vis5d_get_num_of_ctxs_in_display(index, &ctxcnt, cindex);
		  for(i=0;i<ctxcnt;i++){
			 vis5d_enable_graphics(cindex[i], VIS5D_VOLUME, -2, VIS5D_ON);
		  }
		}
#endif
...
#ifdef LG /* SGJ: when is this necessary? it's just a slight shift in
	     window position; the old Makefile uses it on Irix5 */
   gtx->cpy = 32;
#endif
   /*******************************/
...
void create_widgets( int index, int volflag, char *programname )
{
   ...
   /** Row 5 **/
#if defined(MESA) /* defined in GL/gl.h by Mesa */
   if (gtx->animrecBUTTON){
      LUI_NewButtonDestroy(gtx->animrecBUTTON);
   }
   gtx->animrecBUTTON = LUI_ToggleButtonCreate(gtx->CpWindow, 6,
   ...
   if (gtx->how_many_regular_contexts >= 1) {
      ...
      for (yo = 0; yo < gtx->how_many_regular_contexts; yo++){
         ...
         for (i=0; i<gtx->RegularNumVars[spandex]; i++) {
            ...
            yo2 = 0;
            if ( gtx->how_many_regular_contexts > 1){
               while (gtx->RegularVarName[i][spandex][yo2] != 0){
                  yo2++;
               }
               if ((yo2 >= 2 && gtx->RegularVarName[i][spandex][yo2-1] != '.' &&
                                ...
                                yo2 < 2){
                  gtx->RegularVarName[i][spandex][yo2] = '.';
                  gtx->RegularVarName[i][spandex][yo2+1] = 0;
